...
Each boundary crossing needs validation!
```
...
| Password reset       | Token prediction, enum   |
| Registration         | Mass registration, abuse |
| OAuth / SSO          | Token theft, redirect    |
| API key auth         | Key leakage, brute force |
| JWT authentication   | Algorithm confusion      |
| MFA / 2FA            | Bypass, social eng       |
| Session cookies      | Hijacking, fixation      |
| Remember me tokens   | Predictable tokens       |
| Account lockout      | DoS via lockout          |
+----------------------+-------------------------+
```
...
Microservices:
  Browser -> API Gateway -> Service A -> DB-A
                         -> Service B -> DB-B
                         -> Service C -> Cache

Serverless:
  Browser -> CDN -> API Gateway -> Lambda/Functions -> DB

Jamstack:
  Browser -> CDN (static) -> API calls -> Backend services
```
...
| Web Server     | Untrusted |     -     | Semi-trust| N/A  |
| App Server     | Untrusted | Trusted   |     -     | Trust|
| Database       | N/A       | N/A       | Trusted   |  -   |
+----------------+-----------+-----------+-----------+------+
...
4. Are error details exposed?
5. Can this boundary be bypassed?
```
...
| User authentication | Account theft | Critical   |
| Admin panel         | Full control  | Critical   |
| File upload         | RCE           | High       |
| Search function     | SQLi/XSS      | High       |
| API endpoints       | Data exposure | High       |
| Password reset      | Account theft | High       |
| User profiles       | Stored XSS    | Medium     |
| Contact forms       | Spam/XSS      | Medium     |
| Static pages        | Limited       | Low        |
+---------------------+---------------+------------+
```

...
└──────┬──────┘
       ...
       ▼
┌─────────────┐     Priority 10 → mail1.example.com (try first)
│ DNS Server  │──→  Priority 20 → mail2.example.com (fallback)
└─────────────┘     Priority 30 → mail3.example.com (last resort)
```
...
Root (.)              "Who handles .com?"
  │
  ▼
.com NS servers       "Who handles example.com?"
  │
  ▼
example.com NS        "I am authoritative for example.com"
...
│        │←───│ (ISP/8.8.8.8)  │←───│      │←───│      │←───│              │
└────────┘    └─────────────────┘    └──────┘    └──────┘    └──────────────┘
   Single           Does all                 Iterative queries
   query             the work                to each level
```
...
Step 4: Query sent to configured recursive resolver
Step 5: Resolver checks its cache
Step 6: If not cached, resolver walks the DNS hierarchy:
        6a: Query root server → get .com NS
        6b: Query .com NS → get example.com NS
        6c: Query example.com NS → get A record
Step 7: Resolver caches the result and returns it
Step 8: OS caches the result
Step 9: Browser caches the result
Step 10: Browser uses IP to establish TCP connection
```
...
└────────┘  Response  └──────────┘              └────────┘
                ↑
          Attacker can forge
          responses (DNS spoofing)

With DNSSEC:
┌────────┐   Query    ┌──────────┐   Signed      ┌────────┐
│ Client │──────────→ │ Resolver │←─Response────│ NS     │
│        │←──────────│ Validates │              │ Signs  │
└────────┘  Verified  │ signature │              │ records│
            Response  └──────────┘              └────────┘
```

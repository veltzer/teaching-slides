...
│    │─────────────────────────────────>│  Still waiting... │
│    │                                  │                   │
│    │  (repeat for hundreds of         │  All connection   │
│    │   connections simultaneously)    │  slots consumed!  │
└─────────────────────────────────────────────────────────┘
```
...
│    │─────────────────────────────────>│  Expecting body   │
│    │                                  │                   │
│    │  "a"  (1 byte every 10 seconds)  │                   │
│    │─────────────────────────────────>│  Still waiting... │
│    │                                  │                   │
│    │  "b"  (1 byte every 10 seconds)  │                   │
│    │─────────────────────────────────>│  99998 to go...   │
└─────────────────────────────────────────────────────────┘
```
...
│                                                          │
│  ┌──────┐  ┌──────┐  ┌──────┐     ┌─────────────────┐   │
│  │Thread│  │Thread│  │Thread│     │  Event Loop     │   │
│  │ Conn │  │ Conn │  │ Conn │     │                 │   │
│  │  #1  │  │  #2  │  │  #3  │     │ Conn#1 ─┐      │   │
│  │      │  │      │  │      │     │ Conn#2 ─┤ Async │   │
│  │BLOCK │  │BLOCK │  │BLOCK │     │ Conn#3 ─┤      │   │
│  │ ED   │  │ ED   │  │ ED   │     │ ...     ─┘      │   │
│  └──────┘  └──────┘  └──────┘     │ Conn#N          │   │
│                                    └─────────────────┘   │
│  3 slots consumed                  Thousands handled     │
│  by 3 slow connections             by single worker      │
└──────────────────────────────────────────────────────────┘
```
...
│                                                          │
│  Attackers          Reverse Proxy         Backend        │
...
│  │ A ├──┤  Slow    │  proxy       │    │  server    │   │
│  └───┘  │  conns   │              │    │            │   │
│  ┌───┐  │  blocked │  - Timeouts  │    │  Only sees │   │
│  │ A ├──┘  here    │  - Rate limit│    │  complete  │   │
│  └───┘             │  - Conn limit│    │  requests  │   │
│                    └──────────────┘    └────────────┘   │
│                                                          │
│  Slow connections never reach the backend server         │
└─────────────────────────────────────────────────────────┘
```

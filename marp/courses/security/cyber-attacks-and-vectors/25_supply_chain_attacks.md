...
│                                                          │
│  2 years   "Jia Tan" builds trust as xz-utils           │
│  of setup  contributor with legitimate patches           │
│                                                          │
│  Pressure  Social pressure campaign to add Jia Tan       │
│  campaign  as a co-maintainer (sockpuppet accounts)      │
│                                                          │
│  Backdoor  Malicious code hidden in test files           │
│  insertion (.xz compressed binary in test fixtures)      │
│            Injected during build process, not visible    │
│            in source code directly                       │
│                                                          │
│  Target    Backdoor in liblzma -> linked by sshd         │
│            (via systemd) -> RCE via SSH                  │
│                                                          │
│  Discovery Andres Freund noticed 500ms SSH delay         │
│            and investigated (pure luck!)                 │
│                                                          │
│  Impact    Would have backdoored most Linux distros      │
│            Caught just before widespread deployment      │
│                                                          │
│  Lesson    Even open source review can be subverted      │
│            with patient, long-term social engineering     │
└──────────────────────────────────────────────────────────┘
```
...
│  Hosted on private registry (Artifactory, etc.)          │
│                                                          │
│  Build system resolves packages from:                    │
│  1. Private registry (internal)                          │
│  2. Public registry (npm, PyPI) as fallback              │
│                                                          │
│  Attack:                                                 │
│  Attacker publishes "auth-utils" on PUBLIC npm/PyPI      │
│  with a higher version number (e.g., 99.0.0)            │
│                                                          │
│  ┌──────────────┐    ┌─────────────────┐                 │
│  │ Private Reg   │    │ Public Registry │                 │
│  │ auth-utils    │    │ auth-utils      │                 │
│  │ v1.2.3        │    │ v99.0.0         │  <-- MALICIOUS │
│  └──────┬────────┘    └───────┬─────────┘                │
│         │                     │                          │
│         v                     v                          │
│  ┌──────────────────────────────────┐                    │
│  │  Build System: "v99 > v1.2.3"   │                    │
│  │  Installs PUBLIC package!        │  <-- WRONG!        │
│  └──────────────────────────────────┘                    │
│                                                          │
│  Alex Birsan (2021): Compromised Apple, Microsoft,       │
│  PayPal, Netflix, Uber, and others with this technique   │
└──────────────────────────────────────────────────────────┘
```
...
│           (tamper-resistant build records)                │
│                                                          │
│  Level 3: Hardened build platform                        │
│           (isolated, ephemeral, auditable builds)        │
│           (protects against SolarWinds-style attacks)    │
│                                                          │
│  Each level provides increasing confidence that the      │

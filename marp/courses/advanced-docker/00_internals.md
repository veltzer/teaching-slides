...
│              containerd                   │
├──────────┬──────────┬───────────┬────────┤
│ Content  │ Snapshot │ Runtime   │ Image  │
...
2. CLI sends request to dockerd via REST API
       │
3. dockerd pulls image (if needed), creates container record
       │
4. dockerd calls containerd via gRPC
       │
5. containerd creates a task + shim process
       │
6. shim calls runc to create the container
       │
7. runc sets up namespaces, cgroups, mounts
       │
8. runc executes the container entrypoint
       │
9. runc exits, shim takes over as parent
       │
10. Container process is running
```
...
├─────────────────────────────────────┤
│         Lower (image layers)         │
│    layer3 (FROM instruction)         │
│    layer2 (RUN instruction)          │
│    layer1 (base image)               │
└─────────────────────────────────────┘
```

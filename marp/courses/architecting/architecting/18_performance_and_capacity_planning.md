...
- Event-driven invalidation — writers publish, cache evicts; lost events leave cache stale
- Version-bumping (key includes version) — never stale, but capacity policy must reclaim dead keys
- TTL + events — events for freshness, TTL as a safety net when events are lost
...
- Broadcast invalidate works at small scale; coordination cost grows with nodes
- Consistent hashing pins each key to one owner; only that node caches it
- The harder the coherence guarantee, the more the cache resembles a database

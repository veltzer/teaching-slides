...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 │ 4. Building a custom database interface   │
 └──────────────────────────────────────────┘

Goal: Build systems where LLMs can remember
context, use tools, and take autonomous actions.
```
...
AGENT:
  User → LLM → "I need to search the database"
              → [calls search tool]
              → "Found 3 results, let me analyze..."
              → "I also need to check the API"
              → [calls API tool]
              → "Based on my research, here's the answer..."
              → Response to User
  (multi-step, tools, memory, autonomous)
```
...
├────────────────┼────────────────────────────────────┤
│ Plan-and-      │ Create full plan first, then execute│
│ Execute        │ Better for complex multi-step tasks │
│                │ Can revise plan based on results     │
├────────────────┼────────────────────────────────────┤
│ Reflexion      │ Agent reflects on past attempts     │
│                │ Learns from mistakes in-context      │
│                │ Good for trial-and-error tasks       │
├────────────────┼────────────────────────────────────┤
│ LATS           │ Tree search over action space       │
│ (Language      │ Evaluate multiple paths              │
│  Agent Tree    │ Best for complex reasoning           │
│  Search)       │                                      │
└────────────────┴────────────────────────────────────┘
```
...
│            MEMORY STRATEGIES                 │
├──────────────┬──────────────────────────────┤
│ Conversation │ Pass full history in context  │
│ Buffer       │ Simple but limited by context │
├──────────────┼──────────────────────────────┤
│ Summary      │ Summarize old messages         │
│ Memory       │ Saves tokens, loses detail    │
├──────────────┼──────────────────────────────┤
│ Window       │ Keep last N messages           │
│ Memory       │ Fixed token budget            │
├──────────────┼──────────────────────────────┤
│ Vector Store │ Embed and retrieve relevant   │
│ Memory       │ memories semantically          │
├──────────────┼──────────────────────────────┤
│ Entity       │ Track key entities and facts   │
...
└────────┬──────────┘
         │
┌────────▼──────────┐
│  2. SEARCH vector │ → Find similar docs
│     database      │
└────────┬──────────┘
         │
┌────────▼──────────┐
│  3. RETRIEVE top  │ → "Q3 revenue was $4.2M..."
│     k documents   │   "Operating costs were..."
└────────┬──────────┘
         │
┌────────▼──────────┐
│  4. GENERATE      │ → Context + Query → LLM → Response
│     response      │
└────────┬──────────┘
         │
         ▼
"Based on Q3 reports, revenue was $4.2M,
 up 15% from Q2..."
```
...
│           AGENT FAILURE MODES                         │
├──────────────┬───────────────────────────────────────┤
│ LOOPING      │ Agent repeats the same action          │
│              │ Fix: Track action history, detect loops│
├──────────────┼───────────────────────────────────────┤
│ HALLUCINATED │ Agent "uses" tools that don't exist    │
│ TOOLS        │ Fix: Strict tool validation             │
├──────────────┼───────────────────────────────────────┤
│ WRONG TOOL   │ Uses search when calculator needed     │
│ SELECTION    │ Fix: Better tool descriptions           │
├──────────────┼───────────────────────────────────────┤
│ OVER-PLANNING│ Plans forever, never executes          │
│              │ Fix: Force action after N think steps  │
├──────────────┼───────────────────────────────────────┤
│ PREMATURE    │ Answers before gathering enough info   │
│ TERMINATION  │ Fix: Require minimum evidence          │
├──────────────┼───────────────────────────────────────┤
│ CONTEXT      │ Forgets earlier findings in long runs  │
│ OVERFLOW     │ Fix: Summarize + working memory         │
└──────────────┴───────────────────────────────────────┘
```
